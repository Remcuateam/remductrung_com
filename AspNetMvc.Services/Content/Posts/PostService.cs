...
{
    public class PostService : WebServiceBase<Post, int, PostViewModel>, IPostService
    {
        ...
        public override void Update(PostViewModel postVm)
        {
            ...
            var post = Mapper.Map<PostViewModel, Post>(postVm);
            _postTagRepository.Remove(x => x.Id == post.Id);
            if (!string.IsNullOrEmpty(postVm.Tags))
            {
                string[] tags = postVm.Tags.Split(',');
                foreach (string t in tags)
        ...
        public PagedResult<PostViewModel> GetAllPaging(int categoryId, string keyword, int page, int pageSize, string sortBy)
        {
        ...
        public List<PostViewModel> GetListPostByCategoryIdPaging(int categoryId, int page, int pageSize, string sort, out int totalRow)
        {
        ...
        public List<PostViewModel> GetRelatedPosts(int id, int top)
        {
            return _postRepository.GetAll().Where(x => x.Status == Status.Actived
                && x.Id != id)
            .OrderByDescending(x => x.CreatedDate)
